...
import { Scale, FileSearch, Shield, Clock, Quote, ArrowRight, CheckCircle2 } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { ParticleBackground } from '@/components/landing';
import { useTranslation } from '@/i18n';

const BENEFIT_ICONS = [FileSearch, Shield, Clock];
const BENEFIT_KEYS = ['contract', 'diligence', 'time'] as const;

export default function LegalPage() {
  const { t } = useTranslation();
  const features = t('pages.legal.features') as string[];
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="text-center mb-16">
            <span className="inline-flex items-center gap-1 px-4 py-2 rounded-full bg-[var(--landing-primary)]/20 text-[var(--landing-primary)] text-sm font-medium mb-6">
              ...
              {String(t('pages.legal.badge'))}
            </span>
            <h1 className="text-4xl sm:text-5xl font-bold mb-6 text-white">
              AIDORag for <span className="text-[var(--landing-cyan)]">{String(t('pages.legal.title'))}</span>
            </h1>
            <p className="text-xl text-[var(--landing-text-secondary)] max-w-2xl mx-auto mb-8">
              {String(t('pages.legal.subtitle'))}
            </p>
              ...
              {BENEFIT_KEYS.map((benefitKey, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <h3 className="text-lg font-semibold mb-2 text-white">{String(t(`pages.legal.benefits.${benefitKey}`))}</h3>
                    <p className="text-sm text-[var(--landing-text-secondary)]">{String(t(`pages.legal.benefits.${benefitKey}Desc`))}</p>
                  </div>
                );
              })}
            </div>
          ...
          <div className="max-w-3xl mx-auto mb-20">
            <div className="rounded-2xl border-2 border-[var(--landing-primary)] bg-gradient-to-br from-[var(--landing-primary)]/20 to-[var(--landing-purple)]/20 p-8">
              <h2 className="text-2xl font-bold mb-6 text-center text-white">{String(t('pages.legal.capabilities'))}</h2>
          ...
          <div className="max-w-3xl mx-auto mb-20">
            <div className="rounded-2xl border border-[var(--landing-border)] bg-[var(--landing-bg-card)]/50 p-8">
              ...
              <blockquote className="text-lg mb-4 text-white">
                {String(t('pages.legal.testimonial.quote'))}
              </blockquote>
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-[var(--landing-primary)]/20 flex items-center justify-center">
                  <span className="text-sm font-bold text-[var(--landing-cyan)]">JM</span>
                </div>
                <div>
                  <p className="font-semibold text-white">{String(t('pages.legal.testimonial.author'))}</p>
                  <p className="text-sm text-[var(--landing-text-muted)]">{String(t('pages.legal.testimonial.role'))}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="max-w-2xl mx-auto mb-20 text-center">
            <div className="rounded-2xl border border-[var(--landing-border)] bg-[var(--landing-bg-card)]/50 p-6">
              <Shield className="h-8 w-8 text-[var(--landing-cyan)] mx-auto mb-4" />
              <h3 className="font-semibold mb-2 text-white">{String(t('pages.legal.security.title'))}</h3>
              <p className="text-sm text-[var(--landing-text-secondary)]">
                {String(t('pages.legal.security.desc'))}
              </p>
            </div>
          </div>
          ...
          <div className="text-center">
            <h2 className="text-2xl font-bold mb-4 text-white">{String(t('pages.legal.cta.title'))}</h2>
            <p className="text-[var(--landing-text-secondary)] mb-6">
              {String(t('pages.legal.cta.subtitle'))}
            </p>
            <Button
              size="lg"
              className="bg-gradient-to-r from-[var(--landing-primary)] to-[var(--landing-purple)] hover:opacity-90 text-white"
